  ...
  return (
    <>
      <AdminPageHeader
        ...
        description="Operational updates for support, wallet requests, KYC, and other admin queues."
        title="Notifications"
      />

      <AdminCard description="Latest operations updates" title="Admin notification center">
        ...
        {state.kind === "ready" && state.rows.length === 0 ? (
          <EmptyState
            description="New support tickets and review requests will appear here."
        ...
        {state.kind === "setup-required" ? (
          <EmptyState
            description="Generate and apply the pending Drizzle migration before using admin notifications."
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <StatusBadge tone={notification.isRead ? "muted" : "warning"}>
...
function MarkAllReadButton() {
  return (
    <form action={markAllNotificationsReadFormAction}>
      <button
        className="rounded-md border border-border px-3 py-1.5 text-sm text-muted transition duration-150 hover:border-warning/50 hover:text-foreground"
        type="submit"
      >
        Mark all read
      </button>
